/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ServiceStatusWhereUniqueInput } from "./ServiceStatusWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ServiceStatusUpdateInput } from "./ServiceStatusUpdateInput";

@ArgsType()
class UpdateServiceStatusArgs {
  @ApiProperty({
    required: true,
    type: () => ServiceStatusWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ServiceStatusWhereUniqueInput)
  @Field(() => ServiceStatusWhereUniqueInput, { nullable: false })
  where!: ServiceStatusWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ServiceStatusUpdateInput,
  })
  @ValidateNested()
  @Type(() => ServiceStatusUpdateInput)
  @Field(() => ServiceStatusUpdateInput, { nullable: false })
  data!: ServiceStatusUpdateInput;
}

export { UpdateServiceStatusArgs as UpdateServiceStatusArgs };
